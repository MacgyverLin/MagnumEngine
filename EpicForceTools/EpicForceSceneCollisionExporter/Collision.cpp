...
*/
void Collision::setHandler(Collision::Handler *handler_)
{
	handler = handler_;
}
...
*/
Collision::Handler *Collision::getHandler() const
{
	return handler;
}
...
{
	// Collision
	Vector3 resolvepos = getPosition() + info.contactNormal * info.penetrationDepth / 2;
	info.thisObj->setPosition(resolvepos);
}
...
void Collision::Manager::process()
{
	for(int i=0; i<collisions.length(); i++)
	{
		...
		for(int j=i; j<collisions.length(); j++)
		{
			...
			{
				if( body0.test(&body1, info) ) // have method for collision
				{
					...
					if(info.collided)
					{
						...
						// collision handler 0
						if( (body0.getAttribute() & body1.getCategory()) && body0.handler )
							body0.handler->onCollision(info);
						
						info.reverse();

						// collision handler 1
						if( (body1.getAttribute() & body0.getCategory()) && body1.handler )
							body1.handler->onCollision(info);
					}
				}
				else if( body1.test(&body0, info) ) // have method for collision
				{
					if(info.collided)
					{
						info.thisObj  = &body1;
