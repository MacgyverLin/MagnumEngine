...
MultipleCurve2::MultipleCurve2 (int iSegments, float* afTime)
    :
...
const float* MultipleCurve2::GetTimes () const
{
    return m_afTime;
}
//----------------------------------------------------------------------------

void MultipleCurve2::GetKeyInfo (float fTime, int& riKey, float& rfDt)
    const
{
    if (fTime <= m_afTime[0])
    {
...
float MultipleCurve2::GetSpeedWithData (float fTime, void* pvData)
{
...
float MultipleCurve2::GetLength (float fT0, float fT1) const
{
...
float MultipleCurve2::GetTime (float fLength, int iIterations,
...
float MultipleCurve2::GetVariation (float fT0, float fT1,
    const Vector2* pkP0, const Vector2* pkP1) const
{
    ...
    if (iKey0 < iKey1)
    {
        ...
        fVariation += GetVariationKey(iKey1,(float)0.0,fDt1,kA,kB);
    }
    else
    {
        kA = *pkP0 + (m_afTime[iKey0] - fT0)*kB;
