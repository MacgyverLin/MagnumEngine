...
DiffuseMapSpecularMapCubeRenderer::DiffuseMapSpecularMapCubeRenderer(Component::Owner &owner_)
: RendererBase(owner_)
, lightDirections(3)
, lightColors(3)
, angles(3)
...
void DiffuseMapSpecularMapCubeRenderer::add(DiffuseMapSpecularMapCube &cube)
{
	cubes.push() = &cube;
}

void DiffuseMapSpecularMapCubeRenderer::remove(DiffuseMapSpecularMapCube &cube)
{
...
void DiffuseMapSpecularMapCubeRenderer::render(const Video::Manager::RenderParam &param)
{
	for(int i=0; i<3; i++)
	{
	...
	{
		DiffuseMapSpecularMapCube &cube = *cubes[i];
		...
		int specularColorIdx			= visualEffect.getUniformHandle("specularColor");
		int pIdx						= visualEffect.getUniformHandle("p");
		int diffuseMapSamplerIdx		= visualEffect.getTextureHandle("diffuseMapSampler");
		int glossMapSamplerIdx			= visualEffect.getTextureHandle("glossMapSampler");

		TextureBase &texture0			= *visualEffect.getTexture(diffuseMapSamplerIdx);
		TextureBase &texture1			= *visualEffect.getTexture(glossMapSamplerIdx);
		...
		if(visible)
		{
			...
			visualEffect.setValue(specularColorIdx		, Vector4(0.0f, 1.0f, 0.0f, 1.0f)	);
			visualEffect.setValue(pIdx					, 5									);
