...
////////////////////////////////////////////////////////////////////////////////////////
IMPLEMENT_DYNAMIC_CLASS( wxIVector3SpinnerCtrlEvent, wxNotifyEvent )
DEFINE_EVENT_TYPE( wxEVT_COMMAND_IVECTOR3_SPINNER_CTRL_VALUE_CHANGED )

////////////////////////////////////////////////////////////////////////////////////////
wxIVector3SpinnerCtrl::wxIVector3SpinnerCtrl(wxWindow *parent_, wxWindowID id_,
		...
		subPanel->SetSizer(hBox);
			...
			hBox->Add(vBox, 1, wxEXPAND);
				for(int i=0; i<intSpinners.length(); i++)
				{
					...
					intSpinners[i]->Connect(wxEVT_COMMAND_INT_SPINNER_CTRL_VALUE_CHANGED, wxIntSpinnerCtrlEventHandler(wxIVector3SpinnerCtrl::onIntValueChanged)		, 0, this);
				}
	}
	else
	{
...
void wxIVector3SpinnerCtrl::setMinValue(const IVector3 &value_)
{
	for(int i=0; i<intSpinners.length(); i++)
		intSpinners[i]->setMinValue(value_[i]);
}

IVector3 wxIVector3SpinnerCtrl::getMinValue() const
{
	IVector3 rval;
...
void wxIVector3SpinnerCtrl::setMaxValue(const IVector3 &value_)
{
	for(int i=0; i<intSpinners.length(); i++)
		intSpinners[i]->setMaxValue(value_[i]);
}

IVector3 wxIVector3SpinnerCtrl::getMaxValue() const
{
	IVector3 rval;
...
void wxIVector3SpinnerCtrl::setValue(const IVector3 &value_)
{
	for(int i=0; i<intSpinners.length(); i++)
		intSpinners[i]->setValue(value_[i]);
}

IVector3 wxIVector3SpinnerCtrl::getValue() const
{
	IVector3 rval;
...
void wxIVector3SpinnerCtrl::onIntValueChanged(wxIntSpinnerCtrlEvent &evt)
{
	wxIVector3SpinnerCtrlEvent event(wxEVT_COMMAND_IVECTOR3_SPINNER_CTRL_VALUE_CHANGED, GetId());
