		...
		out float2 outputTexCoord2	: TEXCOORD2)
{
	...
	outputTexCoord2		= float2(N.xz * float2(0.5, 0.5) + float2(0.5, 0.5));
}
		...
		out float4 outputColor		: COLOR)
{
	...
	float4 result			= inputColor0 * diffuseColor * splatColor + envMapColor * envAmount;
	outputColor				= float4(result.rgb, 1.0);
}
