...
struct VSOuput
{
	...
	float3 halfVector		: TEXCOORD3; // tangent space halfVector
};
...
PSOuput PS(PSInput input)
{
	...
	float3 N				= 2 * tex2D(bumpMapSampler, input.texCoord0) - 1.0;
	float3 L				= input.lightVector;
	
	output.color  			= float4((ambientLightColor + dirLightColor * blinnDiffuse(L, N)) * diffuseColor, 1);
    return output;
}
